        ...
        ZStack {
            ...
            VStack(spacing: 0) {
                ...
                VStack(alignment: .leading, spacing: 12) {
                    // 1. Safe Area Spacer (pushes content below the status bar/notch)
                ...
                .background(CinemaTheme.deepCharcoal) // 2. Solid background so cards hide behind it
                .zIndex(2) // 3. Ensures header stays on the top layer
                ...
                .padding(.top, 40) // 4. Moves the stack down so it doesn't cover categories
                .padding(.bottom, 20)
                
                Spacer()
            }
            .ignoresSafeArea(edges: .top) // Allows our background to fill the status bar
        }
    }
...
struct CategoryButton: View {
    ...
    var body: some View {
        Button(action: action) {
            // Added background capsule style for a cleaner look
